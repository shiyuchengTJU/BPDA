...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            ...
            mem_label, refined_target_accuracy = obtain_label(dset_loaders['target_test'], netF, netB, netC, args, get_refined_accuracy=True)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset == "VISDA-C":
                ...
                print_log(f"Acc List: {acc_list}", args)
            else:
                acc_target_test, _ = cal_acc(dset_loaders['target_test'], netF, netB, netC)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_extract', type=str, default='ckps/target_more')
    parser.add_argument('--output_target', type=str, default='ckps/target_more_finetune')
    parser.add_argument('--issave', type=bool, default=True)

    parser.add_argument('--no_DEPICT', action="store_true")    # 在目标域模型调优过程中, 除了使用加权聚类外, 是否还使用DEPICT算法进行标签精炼.
